...
pontos_u = rng.uniform(np.pi/3,2*np.pi/3,n_pontos)
pontos_v = rng.uniform(np.pi/3,2*np.pi/3,n_pontos)
...
for i in range(len(pontos_u_iniciais)):
    x = np.sin(pontos_u_iniciais[i])*np.cos(pontos_v_iniciais[i])
    y = np.sin(pontos_u_iniciais[i])*np.sin(pontos_v_iniciais[i])
    z = np.cos(pontos_u_iniciais[i])
    pontos_R3_iniciais[i,:]=np.array([x,y,z])
pontos_R3_finais=np.zeros((len(pontos_u_iniciais),3))
for i in range(len(pontos_u_finais)):
    x = np.sin(pontos_u_finais[i])*np.cos(pontos_v_finais[i])
    y = np.sin(pontos_u_finais[i])*np.sin(pontos_v_finais[i])
    z = np.cos(pontos_u_finais[i])
    pontos_R3_finais[i,:]=np.array([x,y,z])




def runge_kutta_esfera(indice_inicial,indice_final,pontos_u_iniciais,pontos_u_finais,pontos_v_iniciais,pontos_v_finais):
  ...
  while abs(cos_parada)>0.2 and pf_ate_geod>0.01:
    #print(pf_ate_geod)
...
for i in range(len(pontos_u_iniciais)):
  matriz_geod_ida=runge_kutta_esfera(i,i,pontos_u_iniciais,pontos_u_finais,pontos_v_iniciais,pontos_v_finais)
  matriz_geod_volta=runge_kutta_esfera(i,i,pontos_u_finais,pontos_u_iniciais,pontos_v_finais,pontos_v_iniciais)
